(function (global) {
  ...
  async function loadWendaoHistory() {
    ...
    if (!stream) return;
    if (stream.querySelector('.wd-msg')) return;
    const paint = (items) => {
      if (!items || !items.length) return;
      WD_HISTORY_LOADED = true;
      if (typeof global.WD_HISTORY !== 'undefined') global.WD_HISTORY.length = 0;
      items.forEach(it => {
        const q = it.question || '';
        const a = it.answer || '';
        if (typeof global.wdBubble === 'function') {
          global.wdBubble('wd-user', escHtml(q));
          global.wdBubble('wd-master', '<span class="wd-by">先生</span>' + escHtml(a));
        } else {
          stream.innerHTML += '<div class="wd-msg wd-user">' + escHtml(q) + '</div>'
            + '<div class="wd-msg wd-master"><span class="wd-by">先生</span>' + escHtml(a) + '</div>';
        }
        if (typeof global.WD_HISTORY !== 'undefined') {
          global.WD_HISTORY.push({ role: 'user', content: q });
          global.WD_HISTORY.push({ role: 'assistant', content: a });
        }
      });
      try { localStorage.setItem('mingli_wendao_hist_v1', JSON.stringify(items.slice(-40))); } catch (e) {}
    };
    const fromLocal = () => {
      try {
        const raw = localStorage.getItem('mingli_wendao_hist_v1');
        const arr = raw ? JSON.parse(raw) : [];
        if (Array.isArray(arr) && arr.length) paint(arr);
      } catch (e) {}
    };
    try {
      if (!E.askHistory) { fromLocal(); return; }
  ...
  function findCurrentDayun(r, ageXu) {
    if (!r || !r.dayun || !r.dayun.length) return null;
    for (const x of r.dayun) {
      if (ageXu >= x.from && ageXu < x.to) return { gz: x.gz, fromAge: x.from + 1, toAge: x.to, from: x.from, to: x.to };
    }
    return null;
  }
  function yearGZOf(year) {
    const i = ((year - 4) % 60 + 60) % 60;
    const STEMS = '甲乙丙丁戊己庚辛壬癸';
    const BRANCHES = '子丑寅卯辰巳午未申酉戌亥';
    return STEMS[i % 10] + BRANCHES[i % 12];
  }
  function getWendaoContext(askDate) {
    const payload = {};
    ...
    const now = askDate instanceof Date ? askDate : new Date();
    const askYear = now.getFullYear();
    const askMonth = now.getMonth() + 1;
    const askDay = now.getDate();
    if (r && r.year) {
      const b = r._birth || {};
      const birthYear = b.y || r.birthYear || null;
      const ageZhou = birthYear ? (askYear - birthYear) : null;
      const ageXu = birthYear ? (askYear - birthYear + 1) : null;
      let currentDayun = ageXu != null ? findCurrentDayun(r, ageXu - 1) : null;
      // dayun.from is exclusive start age in engine (0-based from); curDayun uses age = year-birthYear
      if (!currentDayun && ageZhou != null) currentDayun = findCurrentDayun(r, ageZhou);
      let currentLiunian = null;
      try {
        if (r.dayGan && birthYear != null && typeof global.E !== 'undefined' && global.E && typeof global.E.liunian === 'function') {
          const age = askYear - birthYear;
          const arr = global.E.liunian(r.dayGan, birthYear, age, age + 1);
          if (arr && arr[0]) currentLiunian = { year: askYear, gz: arr[0].gz, age: arr[0].age };
        }
      } catch (e) {}
      if (!currentLiunian) currentLiunian = { year: askYear, gz: yearGZOf(askYear) };
      payload.baziContext = {
        ...
        dayunList: (r.dayun || []).slice(0, 8).map(x => ({ gz: x.gz, fromAge: x.from + 1, toAge: x.to })),
      };
    } else {
      payload.baziContext = payload.baziContext || {};
      payload.baziContext.askYear = askYear;
      payload.baziContext.askMonth = askMonth;
      payload.baziContext.askDay = askDay;
    }
    return payload;
  }
